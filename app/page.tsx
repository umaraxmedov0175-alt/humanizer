...
export default function Home(){
  ...
  const facts=useMemo(()=>factsOf(source),[source]); const metrics=useMemo(()=>output?scoreText(source,output,level,channel):null,[source,output,level,channel]);
  ...
  async function runRewrite(){
    ...
    setBusy(true);setOutput("");setNotice("");setEngine(null); const opts={channel,audience,level,strength,variation,dialect,warmth,directness,formality,energy};
    try{const r=await fetch("/api/rewrite",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:source,...opts})});const d=await r.json() as {text?:string};if(!r.ok||!d.text)throw 0;setCandidates([d.text]);setOutput(d.text);setEngine("ai")}
    catch{const cs=[0,1,2].map(v=>localRewrite(source,opts,v));setCandidates(cs);setOutput(cs[0]);setCandidate(0);setEngine("local");setNotice("Private local mode · Scores are writing-quality estimates, not authorship probabilities.")}
  ...
  return <main>
    <header><div className="brand"><div className="mark">H</div><span>Humanizer</span><em>studio</em></div><div className="header-meta"><span className="privacy"><i/>Local privacy mode</span><button className="signout" onClick={()=>{sessionStorage.clear();setSignedIn(false)}}>Sign out</button><div className="avatar">U</div></div></header>
    ...
    </section><footer><span>Humanizer improves writing—it does not determine who wrote it.</span><span>Meaning first · No artificial mistakes · No detector evasion</span></footer>
  </main>
}
