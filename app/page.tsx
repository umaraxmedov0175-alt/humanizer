...
export default function Home(){
  ...
  if(!signedIn)return <main className="auth-shell"><section className="auth-story"><div className="auth-brand"><div className="mark">H</div><span>Humanizer 2.0</span></div><div className="auth-message"><p className="eyebrow">CONTROLLED WRITING TRANSFORMATION PLATFORM</p><h1>Transform your writing.<br/><span>Maintain 100% control.</span></h1><p>High-assurance document compilation, claim preservation, and authentic voice refinement.</p></div><div className="auth-proof"><span>✓ Sentinel protected</span><span>✓ Claim ledger verified</span><span>✓ Privacy-first mode</span></div></section><section className="auth-panel"><div className="auth-card"><div className="mobile-auth-brand"><div className="mark">H</div><span>Humanizer 2.0</span></div><p className="step">WELCOME BACK</p><h2>Sign in to your studio</h2><p className="auth-note">Your controlled writing workspace is ready.</p><button className="google-button" onClick={google} disabled={authBusy}><b>G</b>{authBusy?"Opening studio…":"Continue with Google"}</button><div className="divider"><span>or continue with email</span></div><form onSubmit={login}><label>Email address<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com"/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="At least 6 characters"/></label>{authError&&<p className="auth-error">{authError}</p>}<button className="email-button" disabled={authBusy}>Sign in <span>→</span></button></form><p className="terms">Drafts remain in this browser in local privacy mode.</p></div></section></main>;
  return <main>
    <header><div className="brand"><div className="mark">H</div><span>Humanizer 2.0</span><em>platform</em></div><div className="header-meta"><span className="privacy"><i/>Ephemeral privacy mode</span><button className="top-action" onClick={()=>setShowVoice(true)}>Voice profile</button><button className="top-action" onClick={()=>setShowHistory(true)}>History {history.length>0&&<b>{history.length}</b>}</button><button className="signout" onClick={()=>{sessionStorage.clear();setSignedIn(false)}}>Sign out</button><div className="avatar">U</div></div></header>
    <section className="intro"><div><p className="eyebrow">CONTROLLED WRITING TRANSFORMATION PLATFORM</p><h1>High-assurance transformation.<br/><span>Zero factual compromise.</span></h1></div><p className="subhead">Reframe tone, register, and rhythm—while protected sentinels and claims stay 100% intact.</p></section>
    <section className="workspace"><aside className="controls"><div className="control-title"><span>Transformation controls</span><button onClick={()=>location.reload()}>Reset</button></div>
      ...
      <label>Target CEFR level<div className="seg six">{levels.map(x=><button className={level===x?"active":""} onClick={()=>setLevel(x)} key={x}>{x}</button>)}</div><small>Calibrated linguistic complexity estimate</small></label>
      <label>Transformation strength<div className="seg three">{["Light","Balanced","Deep"].map(x=><button className={strength===x?"active":""} onClick={()=>setStrength(x)} key={x}>{x}</button>)}</div></label>
      ...
      <div className="factbox"><div><span className="shield">✓</span><strong>Protected sentinel ledger</strong></div><p>{facts.length?`${facts.length} sentinel spans protected`:"No names, dates, numbers, or quotes detected"}</p><div className="chips">{facts.slice(0,6).map(f=><span key={f}>{f}</span>)}</div></div>
      {sensitive.length>0&&<div className="sensitive"><strong>Privacy notice</strong><p>{sensitive.length} possible sensitive item{sensitive.length>1?"s":""} detected. Review before processing.</p></div>}
    </aside><div className="editor-area"><div className="panes"><article className="pane original"><div className="pane-head"><span>SOURCE DOCUMENT</span><span>{source.split(/\s+/).filter(Boolean).length} words</span></div><textarea value={source} onChange={e=>setSource(e.target.value)} aria-label="Original text"/><button className="clear" onClick={()=>setSource("")}>Clear</button></article><article className="pane result"><div className="pane-head"><span>TRANSFORMED OUTPUT {engine&&<em className={`engine ${engine}`}>{engine === "ai" ? "CLOUD VERIFIED" : "DEGRADED FALLBACK"}</em>}</span>{output&&<button onClick={copy}>{copied?"Copied":"Copy"}</button>}</div>{busy?<div className="thinking"><i/><i/><i/><p>Compiling AST, protecting sentinels, verifying claims…</p></div>:output?<div className="output" contentEditable suppressContentEditableWarning>{output}</div>:<div className="empty"><div className="spark">✦</div><p>Your transformed output will appear here</p><small>Claim ledger verified. Sentinel protected.</small></div>}</article></div>
      {candidates.length>1&&<div className="candidate-bar"><span>Transformation candidates</span>{candidates.map((_,i)=><button className={candidate===i?"active":""} onClick={()=>pick(i)} key={i}>Option {i+1}</button>)}</div>}
      ...
      {notice&&<p className={policyIssue(source)?"policy-notice":"rewrite-notice"}>{notice}</p>}<button className="rewrite" disabled={!source.trim()||busy} onClick={runRewrite}><span>✦</span>{busy?"Verifying claims…":"Transform document"}</button>
    </div></section>
    <section className={`report ${output?"visible":""}`}><div className="report-tabs"><button className={tab==="report"?"active":""} onClick={()=>setTab("report")}>Transformation Quality Vector</button><button className={tab==="changes"?"active":""} onClick={()=>setTab("changes")}>Execution Pipeline</button><button className={tab==="facts"?"active":""} onClick={()=>setTab("facts")}>Sentinel Ledger <span>{facts.length}</span></button></div>
      {metrics&&tab==="report"&&<div className="report-grid"><div className="big-score"><div className="ring"><strong>{metrics.natural}</strong><small>/ 100</small></div><div><h3>{metrics.natural>84?"High-Assurance Accepted":"Accepted with Warnings"}</h3><p>Multi-dimensional quality vector for a {channel.toLowerCase()}; never an authorship probability.</p><span className="confidence">Review Confidence Index: {Math.min(98, metrics.natural)}%</span></div></div><div className="scores"><Score label="Meaning preservation" value={metrics.meaning}/><Score label="Audience fit" value={metrics.audience}/><Score label="CEFR alignment" value={metrics.cefr}/><Score label="Fluency vector" value={metrics.rhythm}/></div><div className={`integrity ${metrics.missing.length?"warn":""}`}><span>{metrics.missing.length?"!":"✓"}</span><div><strong>{metrics.missing.length?"Review factual integrity":"Claim & Sentinel Integrity Passed"}</strong><p>{metrics.missing.length?`Missing or changed: ${metrics.missing.join(", ")}`:"Every protected sentinel span survived output compilation."}</p></div></div></div>}
      {metrics&&tab==="changes"&&<div className="change-list"><div><span>01</span><p><strong>AST Compilation</strong>Parsed document nodes and generated opaque sentinels.</p></div><div><span>02</span><p><strong>Claim Ledger</strong>Extracted exact invariants, entities, and modality relationships.</p></div><div><span>03</span><p><strong>Staged Execution</strong>Generated transformation plan adhering to {level} CEFR constraints.</p></div><div><span>04</span><p><strong>Verification Gate</strong>Validated {facts.length} sentinels and verified claim recall.</p></div></div>}
      {metrics&&tab==="facts"&&<div className="ledger">{facts.length?facts.map(f=><div key={f}><span className={metrics.kept.includes(f)?"kept":"missing"}>{metrics.kept.includes(f)?"Preserved":"Review"}</span><strong>{f}</strong></div>):<p>No protected sentinels detected in input source.</p>}</div>}
    </section><footer><span>Humanizer 2.0 is a Controlled Writing Transformation Platform.</span><span>Meaning first · Claim ledger verified · Ephemeral privacy · <button onClick={eraseData}>Delete my local data</button></span></footer>
